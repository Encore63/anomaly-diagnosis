...
class DELTA(nn.Module):
    ...
    def reset(self):
        ...
        if self.args.optim_type == 'adam':
            self.optimizer = torch.optim.Adam(params, lr=self.args.optim_lr, betas=(self.args.optim_momentum, 0.999),
    ...
    def forward(self, x, target):
        with torch.enable_grad():
            ...
            if self.args.dot is not None:
                with torch.no_grad():
                    self.qhat = self.args.dot * self.qhat + (1. - self.args.dot) * F.softmax(outputs, dim=1).mean(dim=0,
                                                                                                                  keepdim=True)
